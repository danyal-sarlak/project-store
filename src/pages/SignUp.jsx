...
export default function SignUp() {
  ...
  const handleSignUp = async (values) => {
    const newUser = {
      ...
      adress: values.address, // افزودن address به داده‌های ارسالی
    };
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      if (values.address === "") {
        errors.address = "the address field is required"; // افزودن اعتبارسنجی برای address
      }
  ...
  return (
    <form
      className="flex items-center flex-col justify-center h-fit bg-slate-200 p-4"
    ...
    >
      ...
      <div className="w-full max-w-md bg-white rounded-lg h-fit p-5">
        <div className="flex justify-center gap-x-4 pt-6">
          <Link
            to="/login"
            className="flex items-center gap-x-1 justify-center w-20 h-10 bg-orange-400 rounded-lg"
          >
