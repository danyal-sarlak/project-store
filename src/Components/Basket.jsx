...
export default function Basket({ closeBasket }) {
  ...
  const [user, setUser] = useState(null); // برای نگهداری اطلاعات کاربر
  const [isEditing, setIsEditing] = useState(false); // برای کنترل حالت ویرایش
  const [editedName, setEditedName] = useState(""); // برای نگهداری نام ویرایش‌شده
  const [editedAddress, setEditedAddress] = useState(""); // برای نگهداری آدرس ویرایش‌شده
  ...
  const fetchUser = async (token) => {
    ...
    setUser(data);
    setEditedName(data.username); // برای نمایش نام فعلی در input
    setEditedAddress(data.adress); // برای نمایش آدرس فعلی در input
  };
  ...
  const handleSaveClick = async () => {
    const { error } = await supabase
      .from("users")
      .update({ username: editedName, adress: editedAddress }) // اصلاح شده: 'adress' به 'address'
      .eq("id", user.id);
    ...
    setUser((prev) => ({ ...prev, username: editedName, adress: editedAddress }));
    setIsEditing(false);
  };
...
export default function Basket({ closeBasket }) {
  ...
  // Using useQuery with the new object form
  ...
  // Using useMutation with the new object form
  const updateUserMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["user"]); // To invalidate the user query cache
      setIsEditing(false);
    },
    onError: (error) => {
      console.error(error.message);
